...
import Data.Aeson.Fixed
import Data.Aeson.Fixed.TH
...
-- from aeson:
import Data.Aeson.Fixed ( toJSON, Object, object, (.=)
                        , ToJSON, toJSON
                        , FromJSON, parseJSON
                        )
import Data.Aeson.Fixed.Types ( Value(..), Parser )
-- from base:
...
deriveToJSON withField name =
  ...
  where
    ...
    fromCons tvbs cons = do
        --instanceD (return $ (ClassP ''HasResolution [VarT $ mkName "r"]):(map (\t -> ClassP ''ToJSON [VarT (mkName "r"), VarT t]) typeNames))
      r <- newName "r"
      instanceD (let rest = map (\t -> ClassP ''ToJSON [VarT r, VarT t]) typeNames in
                 return ((ClassP ''HasResolution [VarT r]):rest)
                )
                (classType r `appT` instanceType)
                [ funD 'toJSON
                       [ clause []
      ...
      where
        classType r = appT (conT ''ToJSON) (varT r)
...
deriveFromJSON withField name =
  ...
  where
    ...
    fromCons tvbs cons = do
      r <- newName "r"
      instanceD (let rest = map (\t -> ClassP ''FromJSON [VarT r, VarT t]) typeNames in
                 return ((ClassP ''HasResolution [VarT r]):rest)
                )
                (classType r `appT` instanceType)
                [ funD 'parseJSON
      ...
      where
        classType r = appT (conT ''FromJSON) (varT r)
        typeNames = map tvbName tvbs
        instanceType = foldl' appT (conT name) $ map varT typeNames
      ...
      TyConI dec ->
        case dec of
          ...
          other -> error $ "Data.Aeson.TH.withType: Unsupported type: "
                          ++ show other
      _ -> error "Data.Aeson.Fixed.TH.withType: I need the name of a type."
